...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _callback_routes(self) -> List[Dict[str, Any]]:
        """Return configured Telegram callback routes from config.extra."""
        extra = getattr(self.config, "extra", None) or {}
        routes = extra.get("callback_routes", [])
        return routes if isinstance(routes, list) else []

    def _run_callback_route(self, route: Dict[str, Any], callback_data: str) -> bool:
        """Execute one configured callback route. Returns True once matched."""
        prefix = str(route.get("prefix", "") or "").strip()
        if not prefix or not callback_data.startswith(prefix):
            return False

        script_path = str(route.get("script", "") or "").strip()
        name = str(route.get("name", prefix) or prefix)
        if not script_path:
            logger.warning(
        ...
        script = _Path(os.path.expanduser(script_path))
        ...
        timeout_raw = route.get("timeout", 30)
        try:
        ...
        try:
            ...
            logger.warning(
                "[%s] [%s] script timed out after %ss: %s",
                self.name,
                name,
                timeout,
                script,
    ...
    async def _handle_callback_query(
        ...
        for route in self._callback_routes():
            if self._run_callback_route(route, data):
                await query.answer()
                return
                ...
                if str(chat_entry.get("chat_id", "")) == str(chat.id):
                    for topic in chat_entry.get("topics", []):
                        ...
                        if tid is not None and str(tid) == thread_id_str:
                            ...
                            topic_personality = topic.get("personality")
                            break
                    break
